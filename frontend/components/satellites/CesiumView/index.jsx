...
import { faCameraRotate, faArrowRotateRight, faCompress, faHome } from "@fortawesome/free-solid-svg-icons";
...
import ConjunctionView from '../ConjunctionView';
import AnalysisControls from '../AnalysisControls';

const CesiumView = ({
  token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIyMmMyYTBkNS0yMGZkLTQ3YzEtYjY3Yy1hY2M1MWQ5ZTQ0ZDkiLCJpZCI6MzAxMjQ2LCJpYXQiOjE3NDcwNDk1MjZ9.M13g4K52JwC9-AsoKIcMBBpJN0Bxy8PZpkOLD-hZwyI",
  setLoadingStatus,
}) => {
  const dispatch = useDispatch();

  const interpolationDegree = 7;
  const initialClockMultiplier = 1;

  const viewer = useRef({}); // Cesium viewer object reference
  const startDate = useMemo(() => new Date(), []);
  const pointPixelSize = useMemo(() => 1.8, []);
  const userPointPixelSize = useMemo(() => 4.0, []); // Larger size for user satellites

  // categories that will have their objects hidden on load
  const hiddenByDefault = useMemo(() => ["Debris"]);
  ...
  const changeMultiplier = (multiplier) => {
    if (!viewer.current || !viewer.current.entities) return;
  ...
  const handleAnalysisComplete = useCallback(() => {
    ...
    // Refresh conjunction data after analysis is complete
    if (viewer.current) {
      // Trigger a re-render of ConjunctionView
      setKey(prev => prev + 1);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([Cesium, combinedTLE]).then((values) => {
      ...
      // Enable terrain
      viewer.current.terrainProvider = Cesium.createWorldTerrain({
      ...
      viewer.current.resolutionScale = 1.0;  // Increased from 0.8 to 1.0 for better quality
      
      // Adjust camera settings for better Earth visibility
      viewer.current.scene.screenSpaceCameraController.minimumZoomDistance = 1e6;
      viewer.current.scene.screenSpaceCameraController.maximumZoomDistance = 1e9;
      viewer.current.scene.globe.depthTestAgainstTerrain = true;
      
      // Set initial camera position to view Earth
      ...
      propagatedCategories.forEach((category) => {
        category.data.forEach(({position, orbitDuration, name, epochDate, satnum}, i) => {
          const entity = viewer.current.entities.add({
            ...
            point: {
              color: category.color,
              pixelSize: category.name === "User TLE" ? userPointPixelSize : pointPixelSize,
  ...
  return (
    <>
      ...
      {analysisComplete && <ConjunctionView key={key} viewer={viewer.current} />}
      
      {/* Analysis Controls */}
      <div className={styles.analysisControlsContainer}>
        <AnalysisControls onAnalysisComplete={handleAnalysisComplete} />
      </div>
